...
- Focused block view is the default. B01 includes the page overview—Canonical page source/Mockument page heading, metadata, page tools and overall readiness—above the B01 block. B02–B13 hide that overview and render only the selected block beneath the pinned dashboard. Definition and Why it exists reminders live at the bottom of the middle column. The right column is a B12 notes panel: B01 shows all page notes; B02–B13 show only notes related to the active block, with quick-add buttons that bind new notes to that block. A reserved scrollbar gutter prevents movement when block heights differ. Previous/Next controls and URL hashes support sequential and deep-linked navigation.
...
B12 is **Notes**. Every note begins with a Decision, Observation or Question dropdown and reveals only the fields appropriate to that note type. Each note can be tied to a related block. The right-column panel shows all page notes on B01, then filters to the active block on B02–B13; quick-add buttons in that panel create a note already related to the current block. Active decision notes are included in B13’s build contract only when their applied canonical references are recorded.
