...
- Focused block view is the default. B01 includes the page overview—Canonical page source/Mockument page heading, metadata, page tools and overall readiness—above the B01 block. B02–B13 hide that overview and render only the selected block beneath the pinned dashboard. Definition and Why it exists reminders live at the bottom of the middle column. The right column is a page activity log for B12 decisions, observations and questions, with filter counts. A reserved scrollbar gutter prevents movement when block heights differ. Previous/Next controls and URL hashes support sequential and deep-linked navigation.
...
B12 is **Notes**. Every note begins with a Decision, Observation or Question dropdown and reveals only the fields appropriate to that note type. The same records appear in the right-column activity log with All, Decisions, Observations, Questions and Log filters. Active decision notes are included in B13’s build contract only when their applied canonical references are recorded.
